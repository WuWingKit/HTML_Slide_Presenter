...
放映器内置了 5 套风格模板，打开文件后在顶部下拉菜单中选择即可：
...
**模板会自动覆盖幻灯片的颜色、字体、文字大小**，让未写样式的 HTML 也能呈现专业效果。如果你自己写了样式，选择"不使用模板"即可保留原样。
...
**Q: 文字太大/太小？**
模板使用 `vw` 单位自动缩放。如果觉得不合适，可以在文件里自己写样式覆盖。

**Q: 想用自己设计的样式，不要模板？**
在下拉菜单选"不使用模板"，或者在文件的 `<style>` 里写好自己的样式。
